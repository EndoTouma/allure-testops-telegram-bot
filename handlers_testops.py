...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # 2.1) Удаление конкретного проекта (callback_data = delete_<project_id>)
        ...
        # 2.2) Подтверждение запуска (callback_data = launch_confirm)
        if data == "launch_confirm":
            ...
            # Редактируем текст текущего сообщения на «⌛ Запуск Job…»
        ...
        # 2.3) Отмена запуска (callback_data = launch_cancel)
        ...
        if data == "run_test":
            ...
            if not docs:
                text = (
                    ...
                    "Нажмите «➕ Добавить проект», чтобы прикрепить ссылку:\n"
                    "<code>https://YOUR_TESTOPS_URL/projects/123</code>"
                )
        ...
        # 4) Добавить проект (просим ссылку)
        if data == "add_project":
            last_buttons = context.user_data.pop("last_msg_id_with_buttons", None)
            if last_buttons:
                try:
